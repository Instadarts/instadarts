 ...
 * covers every shape that plays today: one user holding any number of players at one shared board,
 * and two users however they split theirs. A third board is a topology nobody has designed yet, and
 * it gets no session rather than a broken one.
 *
...
export function meshEligible(match: MatchState): boolean {
  return boardCount(match.players) <= 2;
}
