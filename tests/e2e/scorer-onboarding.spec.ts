...
//   2. the camera comes first, and opens without a second prompt where access is already granted
//   3. it reaches a verdict rather than hanging or throwing
//   4. the CPU paths never fail — WebGPU may be absent, may fall back, may win
//   5. it settles on some working configuration and says which
//   6. the two reference boards read 8/0 and 8/3 under whatever it settled on
//   7. a phone that cannot have a camera is told why, and can still leave
//
// (6) is also the only thing that checks the downscaled 1280 px images that ship to the client. The
...
test.describe('setting up a scoring device', () => {
  test('a freshly paired phone runs the self-test and ends up able to read a board', async ({ browser }) => {
    ...
    // 2. The camera comes first, and opens by itself because this context already granted access —
    // no second button to press. The preview carrying frames is the proof it really opened.
    await expect(scorer.getByText('Step 1 of 2 · Choosing a camera')).toBeVisible();
    ...
    await expect(scorer.getByText('Step 2 of 2 · Checking what this device can do')).toBeVisible();

    // 3. It reaches a verdict.
    await expect(scorer.getByTestId('onboarding-verdict')).toBeVisible({ timeout: RUN_TIMEOUT });

    // 4 + 5. Some configuration works, and it is the one now stored. A device that gave up says so
    ...
    // 6. The photographs read correctly — under whichever paths this machine settled on.
